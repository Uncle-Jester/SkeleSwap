...
def snap_to_IK(armature):
    ...
    ik_collection = get_bone_collection(armature, "FK_DRIVER_BONES")
    fk_collection = get_bone_collection(armature, "FK_DRIVER_BONES")

    if not ik_collection:
        return
    if not fk_collection:
        return


    for ik_bone in ik_collection:
        fk_bone_name = ik_bone.name.replace("DRV_IK_", "DRV_FK_")
        fk_bone = pose_bones.get(fk_bone_name)
        if fk_bone and (fk_bone in fk_collection):
...
def generate_rig(armature):
    ...
    create_driver_bones(armature, "SNAP_DRIVER_BONES", "DRV_SNAP")
...
class OBJECT_PT_IK_FK_Switch_Panel(bpy.types.Panel):
    bl_label = "IK/FK Switch"
    bl_idname = "OBJECT_PT_IK_FK_Switch_Panel"
...
class OBJECT_OT_generate_rig(bpy.types.Operator):
    ...
    def execute(self, context):
        armature = context.scene.target_armature        
        generate_rig(armature)
        return {'FINISHED'}


# Register functions
classes = [IK_FK_Properties, OBJECT_OT_Switch_IK_FK, OBJECT_PT_IK_FK_Switch_Panel]

def register():
    bpy.utils.register_class(IK_FK_Properties)
    bpy.utils.register_class(OBJECT_OT_Switch_IK_FK)
    bpy.utils.register_class(OBJECT_PT_IK_FK_Switch_Panel)
...
def unregister():
    ...
    bpy.utils.unregister_class(OBJECT_PT_IK_FK_Switch_Panel)
